...
My note: Red, green, blued is absorbed by healthy plants, so plants reflect less of these. Near-infrared is related by the amount of biomass, so it is reflected more.   

# Tasks  
Our goal with this data is to:  
  - Calculate different vegetation indices (combine one or more bands) using the bands  
...
# b:blue; g:green; nir:near infrared; r:red; re:red edge
```
...
# using the red band 
nematode %>% 
  mutate(rndvi = (nir-r)/(nir+r)) # ndvi ranges from O to 1; the higher the value the healthier the crop
...
Let's write a function to return to us the mean and coefficient of variation (CV). If you recall, there is no function for CV in R, so let's also create that.  
...
Standard way of a data frame. We have dataframe with columns. 
```{r}
...
meancv2 <- function(df, col) {
  # Calculate intermediary steps of mean and sd
  mean <-  mean({{df}} %>% pull({{col}}), na.rm = TRUE) # use curly brackets to pull columns; avoids the df or col words 
  sd <- sd({{df}} %>% pull({{col}}), na.rm = TRUE)
...
Since we will be performing the same task across many columns, we could create a loop where, for each column, e.g., variable,   
...
```{r}
nematode_meancv <- data.frame(variable = rep(NA, 26), # create NA's for the 26 variables we calculate 
                              mean = rep(NA, 26),
                              cv = rep(NA, 26)
                              ) 
...
**March 13, 2024**
```{r}
 # index (i) to iterate over vector (from column 3 to 28)
for (i in 3:ncol(nematode_w)) {
  ...
  statsum <- meancv(nematode_w[[i]]) # meancv function takes a vector, not a data frame.
  
  # Appending results to the empty data frame  
  nematode_meancv$variable[[i-2]] <- varname  # i-2 so it starts on first row
...
nematode_mods <- nematode_w %>% 
  ...
  # Pivotting longer for the explanatory variables (remote sensing variables)
  pivot_longer(cols = b:nli, 
               names_to = "exp_var",  #explanatory variable
               values_to = "exp_val") %>%  #explanatory values

  # Creating groups of resp_var and exp_var
  group_by(resp_var, exp_var) %>% 
  nest() %>%   # we can access tibbles within data frame

  # Applying linear model to each element
  mutate(lm = map(data,
                  ~ lm(data = .x, 
                       formula = resp_val ~ exp_val) # nematode_mods$lm[1] gets only one model of 150 
                  ) ) %>% 

  # Extracting R2 for each element  
  mutate(r2 = map(lm, 
                  ~ summary(.x)$r.squared)) # nematode_mods$r2[1] to access r squares
...
nematode_mods_sel <- nematode_mods %>% 
  unnest(r2) %>% group_by(resp_var) %>% summarise(max(r2))
...
nematode_mods_sel_plots <- nematode_mods_sel %>% 
  ...
  mutate(splot = map2(data, r2,
                     ...
                     ~ ggplot(data = .x, 
                              aes(x= exp_val, 
                                  y = resp_val))+
                       geom_point + 
                       geom_smooth(method = "lm")+  #nematode_mods_sel_plots$plot[[1]]
  ...
  mutate(splot_better = pmap(list(.df = data, 
                                  .r2 = r2, 
                                  .rv = resp_var,  #response variable 
                                  .ev = exp_var),  # explanatory variable
...
nematode_mods_sel_plots$splot_better[1] # access a plot
```
...
nematode_mods_sel_plots$plot_better
